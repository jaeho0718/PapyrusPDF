...
// 값 타입(`ReaderPosition`·`ReaderLoadState`·`PendingReaderCommand`·`ReaderSearchState`)은
// 파일 길이 한도로 `PapyrusPDFReaderModel+Types.swift`에 있다.
...
public final class PapyrusPDFReaderModel {
  ...
  /// 현재 선택된 영역의 앵커 사각형입니다 (선택이 없으면 `nil`). 좌표계는
  /// `PapyrusPDFReader`가 차지한 뷰의 좌표계라, 팝오버 부착
  /// (`popover(attachmentAnchor:)`) 같은 곳에 변환 없이 그대로 사용합니다.
  ///
  /// ``selectedRegion``과 같은 통지에서 함께 설정되고 함께 `nil`이 됩니다 —
  /// 선택 해제·문서 교체·텍스트 선택 전환에서도 짝이 어긋나지 않습니다. 값은
  /// **선택 순간의 스냅숏**이며 이후 스크롤·줌을 추적해 갱신하지 않습니다.
  public private(set) var selectedRegionAnchor: CGRect?
  ...
  func attach(core: ReaderCore) {
    ...
    core.onRegionSelectionChange = { [weak self] region, anchorRect in
      self?.selectedRegion = region
      self?.selectedRegionAnchor = anchorRect
    }
