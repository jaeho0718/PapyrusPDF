...
PapyrusPDF는 OCR을 직접 수행하지 않습니다. 대신 `PageTextProvider`를 구현해
`PapyrusPDFReader(document:model:textProvider:)`에 주입하면, 뷰어의 드래그 선택·복사·검색이
내장 텍스트 추출 대신 그 공급원이 내놓는 텍스트 위에서 동작합니다.

```swift
PapyrusPDFReader(document: document, model: model, textProvider: OCRTextProvider(document: document))
```
...
actor OCRTextProvider: PageTextProvider {
  ...
  init(document: PapyrusPDFDocument) {
...
공급원은 문서가 적재되는 시점에 한 번만 포착됩니다. 같은 `PapyrusPDFReader`가 같은
