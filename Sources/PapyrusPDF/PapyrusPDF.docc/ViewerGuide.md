...
`PapyrusPDFReader`와 `PapyrusPDFReaderModel`로 적재 상태를 관찰하고, 목차로 이동하고, 검색
결과를 하이라이트·탐색하고, 읽던 위치를 저장·복원하는 방법입니다. <doc:GettingStarted>의
"뷰어에 표시하기"에서 이어집니다.

## 개요

`PapyrusPDFReaderModel`은 뷰어를 직접 소유하지 않는 관찰·제어용 모델입니다. `PapyrusPDFReader`에
...
모델의 탐색·검색 메서드(`goToPage`, `go(to:)`, `search`, `restore`)는 `PapyrusPDFReader`가
