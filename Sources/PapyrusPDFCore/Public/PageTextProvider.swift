...
///
/// 결과의 좌표 계약은 ``PapyrusPDFDocument/text(forPage:)``와 동일합니다 — ``TextRun/quad``는
...
///
/// ``PapyrusPDFDocument/text(forPage:)`` 위임입니다 (LRU 캐시·중복 요청 병합 포함).
public struct DocumentTextProvider: PageTextProvider {
  /// 공급원 문서입니다.
  public let document: PapyrusPDFDocument
  ...
  /// ``PapyrusPDFDocument/text(forPage:)``에 위임합니다.
  /// - Parameter pageIndex: 조회할 페이지 인덱스입니다 (0 기반).
  /// - Returns: 페이지 텍스트 콘텐츠입니다.
  /// - Throws: ``PapyrusPDFError`` (문서 내장 추출 실패).
