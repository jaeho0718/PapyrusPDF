...
`PapyrusReader`는 별도 설정 없이도 네이티브에 가까운 텍스트 선택을 제공합니다.
...
`PapyrusReaderModel.selection`은 현재 선택을 담은 `@Observable` 프로퍼티입니다.
...
`.papyrusSelectionMenu` modifier로 `PapyrusReader`가 표시할 메뉴 항목을 지정합니다.
