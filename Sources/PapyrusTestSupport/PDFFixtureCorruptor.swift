...
///
/// `PapyrusTestSupport`는 `PapyrusCore`에 의존하지 않으므로(설계 원칙), 이 파일의 xref
/// 관용 스캐너는 `PapyrusCore/XRef/XRefTableParser`와 별개의 자체 구현이다 — 의도된 중복.
...
// MARK: - 클래식 xref 관용 스캐너 (자체 구현, PapyrusCore 비의존)
