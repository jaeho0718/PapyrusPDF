...
public struct TextSelectionContext: Sendable {
  /// 선택 범위입니다.
  ///
  /// `PapyrusCore.TextSelection`으로 정규화된 명칭입니다 — 같은 이름의 SwiftUI 타입과
  /// 충돌하지 않도록 이 파일 안에서는 모듈 한정으로 표기합니다.
  public let selection: PapyrusCore.TextSelection
  ...
  public init(selection: PapyrusCore.TextSelection, selectedText: String) {
...
extension View {
  /// 하위 ``PapyrusReader``의 선택 메뉴 항목을 지정합니다.
  ///
  ...
  public func papyrusSelectionMenu(
    _ items: @escaping SelectionMenuItemsBuilder
  ) -> some View {
    self.environment(\.papyrusSelectionMenu, items)
  }
}

extension EnvironmentValues {
  /// 선택 메뉴 빌더 (미지정 nil — 내부 전파 전용, 공개 표면은 modifier).
  @Entry var papyrusSelectionMenu: SelectionMenuItemsBuilder?
}
