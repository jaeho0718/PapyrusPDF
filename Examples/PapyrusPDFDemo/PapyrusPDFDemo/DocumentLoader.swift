...
///
/// `PapyrusPDFReaderModel`은 문서 하나의 수명에 묶이므로, 문서를 교체할 때마다
/// 새 인스턴스로 갈아 끼운다 (`ContentView`가 새 모델을 `PapyrusPDFReader`에 전달).
@MainActor
@Observable
final class DocumentLoader {
  /// 현재 열린 문서 (미열림이면 `nil`).
  private(set) var document: PapyrusPDFDocument?

  /// 현재 문서에 대응하는 뷰어 모델 (문서 교체 시 새로 만들어진다).
  private(set) var readerModel = PapyrusPDFReaderModel()
  ...
  func open(url: URL) async {
    ...
    await self.load(displayName: url.lastPathComponent) {
      try await PapyrusPDFDocument.open(url: url)
    }
  }
  ...
  func open(data: Data, displayName: String) async {
    await self.load(displayName: displayName) {
      try await PapyrusPDFDocument.open(data: data)
    }
  }
  ...
  func openScannedDemo() async {
    ...
    await self.load(
      ...
      open: { try await PapyrusPDFDocument.open(data: data) }
    )
  }
  ...
  private func load(
    displayName: String,
    provider: ((PapyrusPDFDocument) -> (any PageTextProvider)?)? = nil,
    open: () async throws -> PapyrusPDFDocument
  ) async {
    self.isLoading = true
    defer {
    ...
    do {
      ...
      self.readerModel = PapyrusPDFReaderModel()
      self.registerDemoSelectableRegion(on: self.readerModel)
      self.displayName = displayName
    } catch let error as PapyrusPDFError {
  ...
  private func registerDemoSelectableRegion(on model: PapyrusPDFReaderModel) {
