...
struct ContentView: View {
  ...
  /// 선택 메뉴 커스텀 항목이 남긴 알림 메시지 (M11 `.papyrusPDFSelectionMenu` 데모용).
  ///
  ...
  var body: some View {
    NavigationSplitView {
      ...
      let title = self.loader.displayName.isEmpty ? "PapyrusPDF Demo" : self.loader.displayName
      self.detailContent
  ...
  private var detailContent: some View {
    if self.isGeneratingSyntheticDocument || self.isGeneratingScannedDocument
      ...
      .papyrusPDFSelectionMenu(self.selectionMenuItems)
    } else {
  ...
  /// 실제로 수신·사용해, `.papyrusPDFSelectionMenu`가 항목마다 정확한 컨텍스트를 전달하는지
  /// 눈으로 확인할 수 있다.
    ...
    case .text:
      return [
        SelectionMenuItem(title: "Search Selection", systemImage: "magnifyingglass") { context in
          ...
          // 기존 검색 바 배선(`PapyrusPDFReaderModel.search(_:)`)을 그대로 재사용한다 — 매치
