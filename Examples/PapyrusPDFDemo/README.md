...
PapyrusPDF (`PapyrusPDFUI`/`PapyrusPDFReader`) 뷰어를 실제로 실행·프로파일링하기 위한 데모 앱입니다.
파일 열기, 목차 탐색, 5,000페이지 합성 문서 생성 기능을 제공합니다.

이 프로젝트는 메인 `PapyrusPDF` 스위프트 패키지(`../..`)와는 별도의 Xcode 프로젝트입니다.
`swift build`/`swift test` 대상이 아니므로 패키지 CI에 영향을 주지 않습니다.

## 요구 사항

- Xcode 16 이상 (iOS 18 / macOS 15 SDK)
- 로컬 패키지 참조: `../..` (제품 `PapyrusPDF`)

## 프로젝트 구성 및 빌드

`PapyrusPDFDemo.xcodeproj`는 [XcodeGen](https://github.com/yonaskolb/XcodeGen)으로 `project.yml`에서
...
cd Examples/PapyrusPDFDemo
xcodegen generate
```

XcodeGen은 플랫폼별 타겟(`PapyrusPDFDemo_iOS`, `PapyrusPDFDemo_macOS`)을 생성합니다 — 이 XcodeGen
버전은 단일 타겟에 여러 destination을 부여하는 "멀티플랫폼 앱" 스펙을 지원하지 않아 택한
실용적 대안입니다(두 타겟이 `PapyrusPDFDemo/` 소스를 그대로 공유합니다). 설계 문서가 말하는
...
```sh
cd Examples/PapyrusPDFDemo
xcodebuild -project PapyrusPDFDemo.xcodeproj -scheme PapyrusPDFDemo_macOS \
  -destination 'platform=macOS' -configuration Debug build CODE_SIGNING_ALLOWED=NO

xcodebuild -project PapyrusPDFDemo.xcodeproj -scheme PapyrusPDFDemo_iOS \
  -destination 'generic/platform=iOS Simulator' -configuration Debug build CODE_SIGNING_ALLOWED=NO
```

### Xcode에서 실행

1. `PapyrusPDFDemo.xcodeproj`를 연다.
2. 서명은 "Automatic"(개인 팀)으로 되어 있다 — 실기기 실행 시 본인 Apple ID 팀을 선택한다.
3. 스킴을 `PapyrusPDFDemo_macOS`(Mac) 또는 `PapyrusPDFDemo_iOS`(시뮬레이터/기기)로 선택 후 실행(⌘R).

## 소스 파일

| 파일 | 역할 |
|---|---|
| `PapyrusPDFDemoApp.swift` | `@main` 앱 진입점, `WindowGroup` |
| `ContentView.swift` | `NavigationSplitView`: 사이드바(목차) + 디테일(리더), 열기/생성 툴바, `.papyrusPDFSelectionMenu` 항목(검색·길이 표시·하이라이트 2색·영역 정보) |
| `DocumentLoader.swift` | `@Observable`: `fileImporter` URL/합성 바이트 → `PapyrusPDFDocument.open`, 경고·에러 상태 |
...
`SyntheticPDF`는 `PapyrusPDFTestSupport`(제품 비노출 타겟)를 쓰지 않습니다 — CG의 PDF 그리기
...
4. 목표 미달 시 (난제 3 절차): 프로파일 결과로 병목 특정 → 상수(프리페치 반경·풀 캡·뷰포트
   양자화 노브, `Sources/PapyrusPDFUI/Shared/ReaderLayoutEngine.swift`의
