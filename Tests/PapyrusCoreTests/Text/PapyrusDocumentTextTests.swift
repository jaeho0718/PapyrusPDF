...
/// ``PapyrusDocument/text(forPage:)`` 공개 API 경계 테스트 (설계 §5.5 PA1-PA6).
struct PapyrusDocumentTextTests {
  ...
  @Test func unsupportedFilterThrows() async throws {
    ...
    let document = try await PapyrusDocument.open(data: data)
    do {
...
extension PapyrusDocumentTextTests {
