...
///
/// `PAPYRUSPDF_PERF=1` 환경 플래그 뒤에서만 실행된다 — CI/로컬 기본 실행에서는 스킵된다.
...
/// 193ms), 게이트 판정에 쓰지 않는다 — `PAPYRUSPDF_PERF=1 swift test -c release --filter
...
/// `PAPYRUSPDF_PERF_SCALE`(Double, 부재·비양수 → 1.0)을 곱해 공유 CI 러너의 하드웨어
/// 변동만 흡수한다(§5.8) — 배율 적용 사실은 실패 메시지에 표기한다.
@Suite(
  .enabled(if: ProcessInfo.processInfo.environment["PAPYRUSPDF_PERF"] == "1"),
  .serialized,
  .timeLimit(.minutes(1))
)
struct PerformanceGateTests {
  ...
  /// `PAPYRUSPDF_PERF_SCALE` 예산 배율. 부재·파싱 실패·비양수는 전부 1.0 (배율 미적용).
  private static let perfScale: Double = {
    guard let raw = ProcessInfo.processInfo.environment["PAPYRUSPDF_PERF_SCALE"],
  ...
  @Test func classicTableOpenAndPageCountUnder250Milliseconds() async throws {
    ...
    for _ in 0..<3 {
      ...
      let start = clock.now
      let document = try await PapyrusPDFDocument.open(data: Self.classicTableData)
  ...
  @Test func objectStreamsOpenAndPageCountUnder250Milliseconds() async throws {
    ...
    for _ in 0..<3 {
      let clock = ContinuousClock()
      let start = clock.now
      let document = try await PapyrusPDFDocument.open(data: Self.objectStreamsData)
      _ = try await document.pageCount
      samples.append(clock.now - start)
    }
    let median = Self.median(samples)
  ...
  @Test func fullDocumentSearchCompletesUnder200Milliseconds() async throws {
    ...
    var samples: [Duration] = []
    for _ in 0..<3 {
      let document = try await PapyrusPDFDocument.open(data: searchScanData)
      let clock = ContinuousClock()
      let start = clock.now
