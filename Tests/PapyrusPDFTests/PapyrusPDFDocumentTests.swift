...
/// 공개 퍼사드 ``PapyrusPDFDocument``를 `import PapyrusPDF` 단일 임포트로 검증한다
/// (설계 §5.5 PA1-PA6).
struct PapyrusPDFDocumentTests {
  // MARK: PA1 — 성공 경로 전 표면

  /// PA1: `open(data:)`·`open(url:)` 성공 경로 — pageCount/metadata/outline/page(at:)
  /// 전 표면 + `import PapyrusPDF` 단일 임포트로 컴파일된다.
  @Test func openDataAndURLExposeFullSurface() async throws {
    ...
    let dataDocument = try await PapyrusPDFDocument.open(data: fixture.data)
    ...
    let urlDocument = try await PapyrusPDFDocument.open(url: url)
  ...
  @Test func errorsMapToPublicDomain() async throws {
    await #expect(throws: PapyrusPDFError.notAPDF) {
      _ = try await PapyrusPDFDocument.open(data: Data("not a pdf".utf8))
    }
    ...
    do {
      _ = try await PapyrusPDFDocument.open(data: encrypted.data)
    ...
    do {
      _ = try await PapyrusPDFDocument.open(data: damaged)
    ...
    let missingURL = FileManager.default.temporaryDirectory
      .appendingPathComponent(UUID().uuidString).appendingPathExtension("pdf")
    do {
      _ = try await PapyrusPDFDocument.open(url: missingURL)
  ...
  @Test func warningsMapToPublicKindsAndAreSyncAccessible() async throws {
    ...
    let document = try await PapyrusPDFDocument.open(data: corrupted.data)
  ...
  /// PA6: `catch let e as PapyrusPDFError` 없이 `catch e` 만으로 exhaustive (typed throws 확인).
  @Test func typedThrowsAllowsExhaustiveCatchWithoutCasting() async {
    do {
      _ = try await PapyrusPDFDocument.open(data: Data("not a pdf".utf8))
    } catch {
      // `error`가 이미 `PapyrusPDFError` 타입이다(캐스팅 없이) — 컴파일된다는 사실 자체가 검증.
      let mapped: PapyrusPDFError = error
