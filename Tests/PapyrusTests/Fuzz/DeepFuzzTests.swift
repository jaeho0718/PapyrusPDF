...
/// 로컬/야간 심층 퍼징 — `PAPYRUS_FUZZ=1`이 아니면 스위트 전체가 스킵된다(CI 상시 스위트가
/// 아니다). release 구성 권장:
/// `PAPYRUS_FUZZ=1 PAPYRUS_FUZZ_SEED=0x... swift test -c release --filter DeepFuzzTests`
///
/// 재현 절차(설계 §3.1, §3.5): 마스터 시드는 매 테스트 시작 시 stdout에 출력된다
/// (`PAPYRUS_FUZZ_SEED`로 명시하지 않으면 현재 시각 파생 — 재현하려면 출력된 값을 그대로
/// `PAPYRUS_FUZZ_SEED`에 다시 넣는다). 마스터 시드에서 `SplitMix64`로 파생한 케이스가 실패하면
/// 메시지의 `seed=... mut=0x... n=...`을 `FuzzRegressionTests.knownFindings`에 그대로 옮기면
/// 영구 회귀 케이스가 된다. `PAPYRUS_FUZZ_CORPUS_DIR` 시드(외부 실PDF)는 `FuzzCaseID`로 표현할
...
struct DeepFuzzTests {
  ...
  init() {
    ...
    print(
      "[DeepFuzzTests] root seed = 0x\(Self.hex(seed))"
        + " (재현: PAPYRUS_FUZZ_SEED=0x\(Self.hex(seed)))"
    )
  }

  /// `.openOnly` 표면 — 기본 100,000 케이스 (`PAPYRUS_FUZZ_OPEN_CASES`로 조정).
  ...
  /// `.full` 표면 — 기본 20,000 케이스 (`PAPYRUS_FUZZ_FULL_CASES`로 조정).
  ...
  /// `PAPYRUS_FUZZ_CORPUS_DIR`이 지정된 경우에만 실PDF를 추가 시드로 소비한다(설계 가정 4,
  ...
  @Test func deepCorpusDirFuzzCompletesWithoutCrashesOrHangs() async {
    ...
    guard
      ...
      Issue.record(Comment(rawValue: "PAPYRUS_FUZZ_CORPUS_DIR을 읽을 수 없음: \(corpusDir.path)"))
      return
    }
...
/// `DeepFuzzTests`가 env에서 파싱하는 심층 퍼즈 실행 설정. `PAPYRUS_FUZZ`만 필수 게이트이고
...
struct DeepFuzzConfiguration {
  /// 마스터 시드 — `PAPYRUS_FUZZ_SEED` 미지정 시 현재 시각 파생.
  let rootSeed: UInt64
  ...
  /// `PAPYRUS_FUZZ=1`일 때만 딥 퍼즈 스위트가 실행된다.
  static var isGateEnabled: Bool {
    ProcessInfo.processInfo.environment["PAPYRUS_FUZZ"] == "1"
  }
  ...
  private static func parseEnvironment() -> DeepFuzzConfiguration {
    ...
    let rootSeed = env["PAPYRUS_FUZZ_SEED"].flatMap(Self.parseHexSeed) ?? Self.timeBasedSeed()
    let openCases = env["PAPYRUS_FUZZ_OPEN_CASES"].flatMap(Int.init) ?? 100_000
    let fullCases = env["PAPYRUS_FUZZ_FULL_CASES"].flatMap(Int.init) ?? 20_000
    let timeoutMS = env["PAPYRUS_FUZZ_TIMEOUT_MS"].flatMap(Int.init) ?? 2_000
    let corpusDir = env["PAPYRUS_FUZZ_CORPUS_DIR"].map { URL(fileURLWithPath: $0) }
