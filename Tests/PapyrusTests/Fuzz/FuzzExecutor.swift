...
enum FuzzOutcome: Sendable, Equatable {
  /// 정상 완료 (성공이든 `PapyrusError`든 — 둘 다 "합격"이다).
  case completed
...
extension FuzzExecutor {
  ...
  /// throws가 `PapyrusError` 외 이탈을 컴파일 타임에 봉쇄하므로 에러 값 검사는 불필요하다
  ...
  static func execute(surface: FuzzSurface, input: Data) async {
    switch surface {
    case .openOnly:
      _ = try? await PapyrusDocument.open(data: input)
    case .full:
  ...
  private static func executeFull(input: Data) async {
    guard let document = try? await PapyrusDocument.open(data: input) else {
      return
    }
    ...
    do {
      ...
      // 검색 스트림 에러는 전부 합격 — 실제 타입은 항상 `PapyrusError`다(§4.5).
    }
  }

  /// `page(at:)` 계약 단언 (§4.5): `index`가 `0..<pageCount` 안이므로 `pageOutOfRange`가
  /// 나오면 계약 위반이다 — 퍼즈가 공짜로 주는 검증이라 여기서 바로 단언한다. 그 밖의
  /// `PapyrusError`는 전부 합격.
  private static func assertPageAtHonorsContract(document: PapyrusDocument, index: Int) async {
    do {
      _ = try await document.page(at: index)
    } catch PapyrusError.pageOutOfRange(let outOfRangeIndex, let pageCount) {
      let message = "page(at:) 계약 위반: index \(outOfRangeIndex)는 0..<\(pageCount) 안인데 "
        + "pageOutOfRange를 던짐"
      Issue.record(Comment(rawValue: message))
    } catch {
      // 그 외 PapyrusError는 전부 합격 (§4.5).
    }
  }
}
