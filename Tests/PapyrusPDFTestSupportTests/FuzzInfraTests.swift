...
/// 검증한다 (M8 설계 §2.1-§2.4, 테스트 포인트 1-5). `PapyrusPDFTestSupport`는 `PapyrusPDFCore`에
/// 의존하지 않으므로 이 파일도 원시 바이트 검사만 한다 — 실제 열기 검증은
/// `Tests/PapyrusPDFTests/Fuzz/`가 맡는다.
struct FuzzInfraTests {
  ...
  /// `CoreLimits.maxNestingDepth`(512, `PapyrusPDFTestSupport`는 `PapyrusPDFCore`에 의존하지
