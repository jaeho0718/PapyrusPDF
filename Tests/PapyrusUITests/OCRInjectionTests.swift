...
extension OCRInjectionTests {
  ...
  static func emptyDocument(pageCount: Int) async throws -> PapyrusDocument {
    let fixture = PDFFixtureBuilder(pageCount: pageCount).build()
    return try await PapyrusDocument.open(data: fixture.data)
  }

  /// 내장 텍스트가 있는 1페이지 문서를 연다 (치환 계약 역방향 테스트 전용).
  static func embeddedTextDocument(text: String) async throws -> PapyrusDocument {
  ...
  static func makeSearchableCore(
    document: PapyrusDocument, provider: any PageTextProvider
