...
```swift
/// PDF 문서를 열고 메타데이터·텍스트·검색 기능을 제공하는 진입점입니다.
///
/// 문서는 메모리 맵으로 열리며 전체 파일을 메모리에 올리지 않습니다.
/// 수천 페이지 문서도 열기 비용은 xref 파싱에 비례합니다.
///
/// - Parameters:
...
- **DocC에 렌더링되는 모든 문장은 합니다체로 쓴다.** 카탈로그 문서(랜딩·아티클) 본문과
  public 선언의 `///` 주석 전부가 대상이다(명사형 요약도 "...입니다." 형태로 통일).
  DocC에 렌더링되지 않는 internal/package/`//` 주석은 기존 관례(간결한 평서형)를 유지한다.
